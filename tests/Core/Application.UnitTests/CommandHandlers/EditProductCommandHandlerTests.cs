...
namespace Application.UnitTests.CommandHandlers
{
    public class EditProductCommandHandlerTests
    {
        ...
        [Fact]
        public async Task HandleAsync_Should_UpdateAllFieldsCorrectly()
        {
            // Arrange
            var editProductRequest = new EditProductRequest
            {
            ...
            Assert.True(result.Succeeded);
            Assert.Equal(editProductRequest.Name, result.Data.Name);
            Assert.Equal(editProductRequest.Description, result.Data.Description);
            Assert.Equal(editProductRequest.Price, result.Data.Price);
            Assert.Equal(editProductRequest.StockQuantity, result.Data.StockQuantity);
            Assert.Equal(editProductRequest.IsActive, result.Data.IsActive);
        }
        ...
        public async Task HandleAsync_Should_VerifyUpdateCalledOnce()
        {
            ...
            // Assert
            _repositoryAsyncMock.Verify(x => x.UpdateAsync(It.IsAny<Product>(), CancellationToken.None), Times.Once);
        }
    }
