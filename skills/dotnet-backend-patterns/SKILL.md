...
metadata:
  version: "1.0"
  scope: [root]
  auto_invoke: "Modifying backend application code, architecture, or configuration"
allowed-tools: Read, Edit, Write, Glob, Grep, Bash, WebFetch, WebSearch, Task
---

# .NET Backend Development Patterns

Master C#/.NET patterns for building production-grade APIs, MCP servers, and enterprise backends with modern best practices (2024/2025/2026).
...
#### Fundamental Principles (DDD & Clean Architecture)
1.  **DDD (Domain-Driven Design)**:
    *   **Ubiquitous Language**: Use consistent business terminology in code and documentation.
    *   **Aggregates**: Respect consistency and transaction boundaries.
    *   **Rich Domain Models**: Business logic belongs in the domain, not in application services.
2.  **SOLID**: Apply SRP, OCP, LSP, ISP, and DIP rigorously.
3.  **Dependency Injection**: Use DI to promote decoupling.

#### Responsibility by Layer
*   **Domain**: Root Entities (Aggregates), immutable Value Objects.
*   **Application**: Orchestration of domain operations organized by **Use Cases**.
    *   **Structure**: Each module (e.g., `Pedidos`) must contain its own folder where the following are analyzed and modified: `Commands`, `Queries`, `Validations`, `Mappings`, `Services`.
    *   **Responsibility**: DTOs, input validation, and orchestration. **Must not contain complex business logic**.
*   **Infrastructure**: Implementation of repositories, event bus, external service adapters (Delta, Tango, Datcom).
